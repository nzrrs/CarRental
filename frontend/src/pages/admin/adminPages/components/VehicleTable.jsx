          ...
          <tbody>
            {visible.map((vehicle, idx) => (
              <tr key={vehicle.id} className="border-b border-slate-200 hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-slate-200 rounded-lg flex items-center justify-center text-slate-600 text-xs font-medium">
                      {vehicle.image ? vehicle.image.slice(0, 2).toUpperCase() : "IMG"}
                    </div>
                    <div>
                ...
                <td className="px-6 py-4 text-slate-600">{agencyMap[vehicle.agenceId] || "Unknown"}</td>
                <td className="px-6 py-4">
                  <span className="px-3 py-1 bg-slate-100 text-slate-700 rounded-full text-xs font-medium border border-slate-200">
                    {categories?.find((c) => c.id === vehicle.categorieId)?.nom || "Unknown"}
                  </span>
                </td>
                <td className="px-6 py-4 font-medium text-slate-900">{vehicle.prixJournalier} MAD</td>
                <td className="px-6 py-4">
                  <StatusIndicator available={vehicle.estDisponible} />
                </td>
                <td className="px-6 py-4">
                  <ValidationBadge status={vehicle.estDisponible ? "approved" : "pending"} />
                </td>
              </tr>
            ))}
            {visible.length === 0 && (
